...
static struct osd_dev *_ios_od(struct ore_io_state *ios, unsigned index)
{
	ORE_DBGMSG2("oc->first_dev=%d oc->numdevs=%d i=%d oc->ods=%p\n",
		    ...
		    ios->oc->ods);
	//oc = object compomnet (ore_component*) -> 여러 device를 저장
...
//object request 생성. 
//ios는 per_dev[]를 가지고, 그 안엔 bio, obj_request가 있다.
//즉 ios는 여러 device로 향하는 bio(input)과 obj_reqeust(output)이 있고, 이를 변환하는 과정.
//
int ore_create(struct ore_io_state *ios)
{
	int i, ret;
	//per_device마다..여기서 device 의미는..?
	for (i = 0; i < ios->oc->numdevs; i++) {
		struct osd_request *or;
		
		//osd_dev* _ios_od(ios, i)는 i번째 device를 리턴함
